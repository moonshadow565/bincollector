...
namespace fltbf {
    ...
    inline from_offset(Offset offset, T& value) {
        ...
        T result;
        fltbf_assert(offset.cur >= 0 && offset.cur + static_cast<int32_t>(sizeof(T)) <= offset.end);
    ...
    inline void from_offset(Offset offset, std::u8string& value) {
        ...
        memcpy(value.data(), offset.beg + offset.cur, static_cast<size_t>(size));
    }
    ...
    inline void from_offset(Offset offset, Table& value) {
        ...
        value.offsets.resize(static_cast<size_t>(members_size / 2));
        memcpy(value.offsets.data(), offset.beg + offset.cur, static_cast<size_t>(members_size));
    }
    ...
    inline void from_offset(Offset offset, std::vector<T>& value) {
        ...
        if constexpr (std::is_arithmetic_v<T> || std::is_enum_v<T>) {
            fltbf_assert(offset.cur + size * static_cast<int32_t>(sizeof(T)) <= offset.end);
            value.resize(static_cast<size_t>(size));
            memcpy(value.data(), offset.beg + offset.cur, static_cast<size_t>(size) * sizeof(T));
        } else {
            fltbf_assert(offset.cur + size * static_cast<int32_t>(sizeof(int32_t)) <= offset.end);
            value.resize(static_cast<size_t>(size));
            for (auto& item: value) {
